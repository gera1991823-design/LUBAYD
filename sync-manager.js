...
// Procesador secuencial de la cola offline.
(() => {
  let running = false;
  ...
  async function process(options) {
    ...
    try {
      ...
      await onChange({ running: true, total, remaining: total });
      ...
      for (let index = 0; index < items.length; index += 1) {
        ...
        try {
          ...
          await OfflineDB.updateQueue(item);
          failed += 1;
          console.error("Sincronización pendiente:", item.type, error);
          // Se continúa con los demás registros para que un fallo aislado no bloquee toda la cola.
        }
  ...
  window.LubaydSyncQueue = { process, isRunning };
})();
